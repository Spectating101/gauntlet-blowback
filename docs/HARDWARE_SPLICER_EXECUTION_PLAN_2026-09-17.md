...
1. `HS-EW-01` — inspect the live Embedded World submission form and record exact requirements.
2. `HS-EW-02` — draft and claim-audit the talk package by September 20.
3. `HS-PKG-01` — freeze the positioning hypothesis, audience matrix, and canonical message hierarchy.
4. `HS-PKG-02` — build the landing page, quickstart, visual evidence map, copy library, and downloadable evaluator kit.
5. `HS-PKG-03` — produce the offline demonstration and packaging QA report.
6. `HS-ART-01` — inventory all release contents by origin, ownership, redistribution status, and proposed license.
7. `HS-ART-02` — generate the archive manifest, `CITATION.cff` draft, and Zenodo metadata preview without publishing.
8. `HS-EVAL-01` — freeze the evaluation question, baseline, scenario taxonomy, and metrics.
9. `HS-PHY-01` — validate the provider-neutral quotation package without contacting or paying a vendor.
10. `HS-PLAN-01` — convert verified routes into machine-readable Gauntlet records and prevent paused Shih Hsin work from displacing this campaign.
