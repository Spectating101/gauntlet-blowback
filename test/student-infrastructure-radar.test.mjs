...
const historicalMaster = new Map(buildMasterRegistry({ includeArchived: true }).map((route) => [route.id, route]));
...
test('master builder integrates the new infrastructure tranche and refreshes OpenAI Researcher Access', () => {
  for (const id of routes.keys()) assert.ok(historicalMaster.has(id), `historical master missing infrastructure route ${id}`);
  const openai = historicalMaster.get('openai-researcher');
  assert.ok(openai, 'missing OpenAI Researcher Access legacy route');
  assert.equal(master.has('openai-researcher'), false);
...
test('Sep-4 research/resource tranche enters the Gauntlet master with explicit execution gates', () => {
  ...
  for (const id of [
    ...
    assert.equal(master.has(id), !archived, `${id}: active archive state mismatch`);
    assert.ok(historicalMaster.has(id), `historical master missing Sep-4 route ${id}`);
  }

  const awsCfp = historicalMaster.get('aws-community-day-taiwan-2026-hardware-splicer');
  ...
  const nchc = historicalMaster.get('nchc-university-ai-compute-yzu-2026');
...
test('Sep-4 candidate assessment uses canonical Gauntlet ids and contains no stale aliases', () => {
  ...
  for (const id of canonicalIds) {
    ...
    assert.ok(historicalMaster.has(id), `canonical candidate id ${id} is not represented in historical master`);
  }
...
test('Sep-4 overrides keep high-upside routes from outrunning eligibility or evidence', () => {
  ...
  const science = historicalMaster.get('anthropic-ai-for-science-general-2026');
  ...
  const awsResearch = historicalMaster.get('aws-cloud-credit-research-2026');
...
test('application compiler distinguishes benefit claims, research credits and PI dependencies', () => {
  ...
  const anthropic = historicalMaster.get('anthropic-ai-for-science-general-2026');
  ...
  const google = historicalMaster.get('google-cloud-research-credits-pi-2026');
