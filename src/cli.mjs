...
if (command === 'fire') {
  if (!filePath) { usage(); process.exit(2); }
  const { fireHandoffForRoute } = await import('./application/fire.mjs');
  console.log(JSON.stringify(await fireHandoffForRoute(filePath), null, 2));
  process.exit(0);
}

if (command === 'fire-queue') {
  const args = [filePath, ...rest].filter(Boolean);
  const { fireHandoffQueue } = await import('./application/fire.mjs');
  console.log(JSON.stringify(await fireHandoffQueue(undefined, runtimeFireOptions(args)), null, 2));
  process.exit(0);
}

if (command === 'fire-receipt') {
  if (!filePath) { usage(); process.exit(2); }
...
if (command === 'calendar-queue') {
  const args = [filePath, ...rest].filter(Boolean);
  const take = (name, fallback) => {
    const raw = args.filter((arg) => arg.startsWith(`${name}=`)).at(-1);
    return raw ? raw.slice(name.length + 1) : fallback;
  };
  const { buildCalendarOnboardingQueue } = await import('./application/calendar-onboarding.mjs');
  console.log(JSON.stringify(buildCalendarOnboardingQueue({
...
if (command === 'account-checkpoint') {
  if (!filePath) { usage(); process.exit(2); }
  const raw = JSON.parse(await fs.readFile(filePath, 'utf8'));
  const { persistAccountCheckpoint } = await import('./application/calendar-onboarding.mjs');
  console.log(JSON.stringify(persistAccountCheckpoint(raw), null, 2));
  process.exit(0);
}
...
if (command === 'readiness-audit') {
  ...
  const { auditApplicationReadiness } = await import('./application/readiness-audit.mjs');
  console.log(JSON.stringify(await auditApplicationReadiness({
    today: take('--date', new Date()),
